...
def cross_game_resonance(
    ...
    if not target_game:
        return magnetic_resonance(query_text, entities, top_k)

    # 先找同游戏的记忆
    same_game = [e for e in entities if identify_game(e.get("text", "")) == target_game]
    other_game = [e for e in entities if identify_game(e.get("text", "")) != target_game]

    # 同游戏优先
    results = magnetic_resonance(query_text, same_game, top_k)

    # 补充跨游戏相关记忆
    if len(results) < top_k:
        cross_results = magnetic_resonance(query_text, other_game, top_k - len(results))
        # 标记为跨游戏
...
if __name__ == "__main__":
    # 测试用例
    test_entities = [
        {"id": "bug-001", "text": "贪吃蛇碰撞检测Bug：自碰判定有误"},
        {"id": "bug-002", "text": "塔防炮塔升级逻辑错误"},
        {"id": "bug-003", "text": "火柴人格斗hitbox偏移"},
        {"id": "bug-004", "text": "苍穹射击子弹碰撞穿透"},
    ]
    ...
    results = magnetic_resonance(query, test_entities, top_k=3)
    for r in results:
        print(f"  [{r['_score']:.4f}] {r['id']}: {r['text']}")

    print()
    print("跨游戏匹配:")
    cross_results = cross_game_resonance(query, test_entities, top_k=3)
    for r in cross_results:
        cross_tag = " [跨游戏]" if r.get("_cross_game") else ""
        print(f"  [{r['_score']:.4f}] {r['id']}: {r['text']}{cross_tag}")
